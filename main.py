...
async def cmd_start(message: types.Message):
    ...
    await message.answer(COMMANDS_MSG['start'] + message.from_user.full_name + "!")
    await set_default_commands(dp)

@dp.message_handler(commands=['stop'])
async def cmd_stop(message: types.Message):
    mybot.unregister_user(message.from_user)
    await message.answer(COMMANDS_MSG['stop'] + message.from_user.full_name + "!")
...
async def tag(message: types.Message):
    ...
    tag_users_on = [k for k in tag_users if (k in mybot.users.keys()) and (mybot.users.get(k) is not None)]
    if len(tag_users_on) > 0:
        await message.reply(", ".join(tag_users_on) + " 👆")
